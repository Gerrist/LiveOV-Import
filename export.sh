...
mongo liveov --eval 'db.calls.createIndex( { "trip": 1, "date": 1, "stop": 1, "departureTime": 1 } ); db.trips.createIndex( { "trip": 1, "date": 1 } ); db.stops.createIndex( { "stop": 1, "stopArea": 1} ); db.stopareas.createIndex( { "code": 1} );'
mongoimport --db liveov --collection stopareas --type csv --headerline --file out/stop-areas.csv --upsert --upsertFields code
mongoimport --db liveov --collection stops --type csv --headerline --file out/stops.csv --upsert --upsertFields code
